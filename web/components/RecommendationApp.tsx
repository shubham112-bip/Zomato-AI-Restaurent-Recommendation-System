...
export function RecommendationApp() {
  ...
  const applyTag = (tag: string) => {
    if (tag === "Near Me") {
      setCravings((c) => (c ? `${c}, nearby` : "nearby"));
      return;
    }
    setCuisine(tag);
  };
  ...
  const buildExtras = () => {
    const parts: string[] = [];
    if (naturalQuery.trim()) parts.push(naturalQuery.trim());
    if (cravings.trim()) parts.push(cravings.trim());
      ...
      <section className="relative min-h-[520px] overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
          ...
          <form onSubmit={onSubmit} className="rounded-2xl bg-white p-6 shadow-xl sm:p-8">
            ...
            <div className="mt-6 flex gap-2 rounded-xl border border-zinc-200 bg-zinc-50 p-1 pr-1">
              <input
                type="text"
                className="min-w-0 flex-1 rounded-lg bg-transparent px-3 py-2.5 text-sm text-[#1C1C1C] outline-none placeholder:text-zinc-400"
              ...
              <button
                type="button"
                className="rounded-lg p-2 text-[#E23744] hover:bg-zinc-200/80"
                aria-label="Voice search (decorative)"
              >
              ...
              <button
                type="submit"
                className="rounded-lg bg-[#E23744] px-4 py-2 text-sm font-semibold text-white hover:bg-[#c42e3a]"
              >
                Send
              </button>
            </div>
